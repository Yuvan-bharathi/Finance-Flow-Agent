  ...
  if (name === 'getLoanScheduleStatus') {
    const [loans] = await pool.query(`
      SELECT l.*, c.company_name,
             ...
             SUM(CASE WHEN rs.status = 'overdue' THEN (rs.scheduled_amount - rs.paid_amount) ELSE 0 END) AS total_overdue_amount,
             SUM(CASE WHEN rs.status IN ('pending', 'overdue') THEN (rs.scheduled_amount - rs.paid_amount) ELSE 0 END) AS total_pending_amount
      FROM loans l
