...
interface TokenStats {
  ...
  today_cost_inr: string;
  active_model: string;
  available_models: TokenModel[];
  agent_breakdown: AgentTokenBreakdown[];
}
...
export const Settings = () => {
  ...
  const [creationSuccess, setCreationSuccess] = useState<CreatedUserResponse | null>(null);
  ...
  const [matchingRules, setMatchingRules] = useState<Record<string, boolean>>({
    bankAccount: true,
    amount: true,
    reference: true,
    schedule: true,
    ...
    const loadSettings = async () => {
      try {
        ...
        if (sysPrefs.confidence_threshold !== undefined)
          setPreCheckThreshold(parseInt(String(sysPrefs.confidence_threshold), 10));
        if (sysPrefs.agent_1_enabled !== undefined)
          setAutoPaymentAnalysis(sysPrefs.agent_1_enabled === 'true');
      } catch (err: unknown) {
        console.warn('[Settings] Could not load settings from MySQL:', (err as Error).message);
      } finally {
  ...
  const fetchUsers = async () => {
    try {
      ...
      setUsersList((res.data?.data?.users || []) as TeamUser[]);
    } catch (err) {
  ...
  const fetchTokenUsage = async () => {
    try {
      ...
      console.warn('[Settings] Failed to load token usage:', (err as Error).message);
    } finally {
  ...
  const handleSwitchModel = async (newModel: string) => {
    try {
      ...
      console.error('[Settings] Model switch error:', (err as Error).message);
    } finally {
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setCreationSuccess(res.data?.data?.user || null);
      setNewUserName('');
      setNewUserEmail('');
      await fetchUsers();
    } catch (err: unknown) {
      setCreationError((err as { response?: { data?: { message?: string } } })?.response?.data?.message || 'Failed to create user');
    } finally {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div style={{ background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '16px', padding: '24px' }}>
            ...
            <div style={{ marginTop: '16px', background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: '12px', padding: '14px', fontSize: '0.8rem', color: '#1e40af', lineHeight: 1.4 }}>
              <strong>Operational Policy:</strong> Cases scoring <strong>&ge; {preCheckThreshold}%</strong> can receive a rule-based recommendation without an LLM call. Human approval is still required before financial settlement.
            </div>
          </div>
          ...
          {isSuperAdminOrOwner && (
            <div style={{ background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '16px', padding: '24px', boxShadow: '0 2px 8px rgba(0,0,0,0.02)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(210px, 1fr))', gap: '14px', marginBottom: '20px' }}>
                ...
                <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '12px', padding: '16px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                    <span style={{ fontSize: '0.75rem', fontWeight: '700', color: '#64748b' }}>Estimated Cost</span>
              ...
              <div style={{ marginBottom: '20px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '12px' }}>
                  {(tokenStats.available_models.length > 0 ? tokenStats.available_models : [
              ...
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '0.85rem', color: '#0f172a' }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                    <input type="radio" name="overpay" value="manual" checked={overpaymentStrategy === 'manual'} onChange={e => setOverpaymentStrategy(e.target.value)} />
                    <span>Send for Manual Accountant Review <strong>(Default Safe)</strong></span>
                  </label>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                    <input type="radio" name="overpay" value="next" checked={overpaymentStrategy === 'next'} onChange={e => setOverpaymentStrategy(e.target.value)} />
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '0.85rem', color: '#0f172a' }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                    <input type="radio" name="underpay" value="manual" checked={underpaymentStrategy === 'manual'} onChange={e => setUnderpaymentStrategy(e.target.value)} />
