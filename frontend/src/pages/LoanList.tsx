...
export interface EnrichedLoanSchedule extends LoanSchedule {
...
export interface EnrichedLoan extends Loan {
  ...
  remaining_scheduled_balance?: number | string;
  ...
  schedules?: EnrichedLoanSchedule[];
}
 ...
 */
const formatINR = (val: string | number | undefined, compact = false) => {
...
export const LoanList = ({ onAskAI }: LoanListProps) => {
  const { user } = useAuth();
  ...
  const fetchLoans = useCallback(async () => {
    try {
      ...
      setLoans((res.data?.data || []) as EnrichedLoan[]);
      const compRes = await api.get('/companies');
      setCompanies((compRes.data?.data || []) as Company[]);
    } catch (err) {
  ...
  const handleFetchLoanDetails = async (loanId: number) => {
    try {
      setLoadingSchedule(true);
      const res = await api.get(`/loans/${loanId}`);
      setSelectedLoan(res.data?.data || null);
    } catch (err) {
  ...
  const handleTriggerCollection = async (loan: EnrichedLoan) => {
    const overdueAmt = parseFloat(String(loan.overdue_amount || 0));

    if (overdueAmt <= 0) {
      setToastMessage({
        type: 'info',
        text: `✓ ${loan.company_name || 'Borrower'} has no overdue installments. Facility is in good standing (₹0.00 Overdue).`,
      });
    ...
    try {
      ...
      setToastMessage({
        type: 'error',
        text: `Notice generation notice: ${msg}`,
      });
      setTimeout(() => setToastMessage(null), 5000);
    } finally {
  ...
  const handleCreateLoan = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (status === 403 || status === 401) {
        window.dispatchEvent(new CustomEvent('ff-auth-permission-error', {
          detail: { status: status || 403, message: (err as { response?: { data?: { message?: string } } })?.response?.data?.message || 'Access denied: You do not have permission to create loans.' },
        }));
      } else {
        alert((err as { response?: { data?: { message?: string } } })?.response?.data?.message || 'Loan creation failed');
      }
    } finally {
      setSubmitting(false);
    }
  ...
  const portfolioKPIs = useMemo(() => {
    ...
    loans.forEach(l => {
      const principal = parseFloat(String(l.principal_amount || 0));
      const scheduled = parseFloat(String(l.total_scheduled_amount || l.total_payable || 0));
      const paid = parseFloat(String(l.total_paid_amount || 0));
      const rem = parseFloat(String(l.remaining_scheduled_balance || 0));
            ...
            <tbody>
              {loading ? (
                ...
                filteredLoans.map(l => {
                  ...
                  const remaining = parseFloat(String(l.remaining_scheduled_balance || 0));
                  const overdueAmt = parseFloat(String(l.overdue_amount || 0));
                    ...
                    >
                      ...
                      <td style={{ padding: '14px 20px' }}>
                        ...
                        <div style={{ fontSize: '0.75rem', color: '#475569', marginTop: '2px' }}>
                          @{parseFloat(String(l.interest_rate))}% p.a. • <strong>{totalInst} Months</strong>
                        </div>
                      ...
                      <td style={{ padding: '14px 20px', textAlign: 'right' }}>
                        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', justifyContent: 'flex-end' }}>
                          <button
                            onClick={() => void handleFetchLoanDetails(l.id)}
          ...
          >
            ...
            <div style={{ padding: '16px 24px', background: '#ffffff', borderBottom: '1px solid #f1f5f9', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px' }}>
              ...
              <div>
                ...
                <div style={{ fontSize: '1rem', fontWeight: '800', color: '#4f46e5' }}>{parseFloat(String(selectedLoan.interest_rate))}% p.a.</div>
              </div>
              <div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem', textAlign: 'left' }}>
                  ...
                  <tbody>
                    {selectedLoan.schedules.map(s => {
                      ...
                      const isPartial = s.status === 'partially_paid' || (parseFloat(String(s.paid_amount || 0)) > 0 && parseFloat(String(s.paid_amount || 0)) < parseFloat(String(s.scheduled_amount)));
                      const isOverdue = !isPaid && !isPartial && (s.days_overdue ?? 0) > 0;
                      const remaining = parseFloat(String(s.remaining_amount || 0));
