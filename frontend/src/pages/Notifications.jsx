...
import {
  ...
  Check, X, ChevronRight, ChevronDown, ChevronUp, AlertCircle, Bot,
  Mail, Copy, UserCheck, ShieldCheck, CheckSquare, Square, SendHorizontal,
  Building, User, AlertOctagon, FileCheck
} from 'lucide-react';
import {
  getAlerts,
 ...
 * Categorized by Executive Escalations (Agent 6) vs Borrower Follow-Up Notices (Agent 3).
 */
export const Notifications = ({ onAskAI, onSelectCase }) => {
  ...
  // Explicitly authorized: Owner, Super Admin, Admin, Manager, Senior Accountant
  ...
  const [categoryTab, setCategoryTab]   = useState('all'); // all | escalation | collection
  ...
  // ─── Fetch Alerts from Backend (Sorted by created_at DESC) ─────────────────
  ...
  // ─── Human Action: Single Approve Alert & Dispatch Email ───────────────────
  const handleApprove = async (alertOrId) => {
    if (isViewer || !isAuthorized) {
      window.dispatchEvent(new CustomEvent('ff-auth-permission-error', {
  ...
  // ─── Human Action: Single Dismiss Alert ────────────────────────────────────
  ...
  // ─── Human Action: Batch Approve & Dispatch All / Selected ─────────────────
  const handleBatchApprove = async (targetAlertIds = null) => {
    if (isViewer || !isAuthorized) {
      window.dispatchEvent(new CustomEvent('ff-auth-permission-error', {
    ...
    try {
      ...
      const count = res.data?.data?.count || idsToApprove.length;
  ...
  // ─── Human Action: Batch Dismiss Selected ──────────────────────────────────
  ...
  const handleCopyDraft = (text, alertId) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      {/* ── Category Navigation Tabs (Eliminates mingling of internal escalations vs borrower emails) ── */}
      <div style={{
        display: 'flex',
        alignItems: 'center',
        gap: '12px',
      ...
      <div style={{
        ...
        {/* Right Batch Operations Actions */}
        {pendingAlertList.length > 0 && isAuthorized && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
            ...
            {/* If items selected: Dispatch Selected */}
            ...
            {/* Explicit Button: Batch Dispatch All Pending Mails */}
            <button
              onClick={() => handleBatchApprove(pendingAlertList.map(a => a.id))}
              disabled={batchLoading}
      ...
      {/* ── Notification Alert Cards List (Latest at Top) ──────────────────── */}
      <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
        {loading ? (
          ...
          <div style={{
            ...
            <ShieldAlert size={32} style={{ margin: '0 auto 12px auto', display: 'block', color: '#94a3b8' }} />
          ...
          filteredAlerts.map(alert => {
            ...
            // Generate structured collection notice email draft body
            ...
            return (
              ...
              >
                {/* Top Row: Checkbox, Channel Badge, Severity, Status, Routing & Local Timestamp */}
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '10px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                    ...
                    {/* Explicit Classification Pill: Internal vs Borrower */}
                    <span style={{
                ...
                <div style={{
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '0.95rem', fontWeight: '900', color: alert.overdue_days > 60 ? '#b91c1c' : alert.overdue_days > 30 ? '#d97706' : '#475569', marginTop: '2px' }}>
                ...
                {/* ── EXPANDED EXECUTIVE EMAIL NOTICE DRAFT PREVIEW (High Contrast Visible Text) ── */}
                {isExpanded && (
                  <div style={{
                    background: '#f8fafc',
                    ...
                    {/* Email Headers Card with high contrast dark text */}
                    <div style={{
                      background: '#ffffff',
                      border: '1px solid #cbd5e1',
                      borderRadius: '8px',
                      ...
                      <div>
                        <strong style={{ color: '#475569' }}>Escalation Route:</strong>{' '}
                    ...
                    {/* Formatted Message Body Container with crisp visible dark text */}
                    <div style={{
                      background: '#ffffff',
                      border: '1px solid #cbd5e1',
                      borderRadius: '8px',
                  ...
                  {isPending && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <button
                        ...
                        disabled={actionLoading[alert.id] || !isAuthorized}
                        title={!isAuthorized ? 'Read-only role — dismiss restricted' : 'Dismiss Alert'}
                        style={{
                      ...
                      <button
                        ...
                        disabled={actionLoading[alert.id] || !isAuthorized}
                        title={!isAuthorized ? 'Role restricted — approval requires Owner, Admin, Manager, or Senior Accountant' : 'Approve & Dispatch Single Notice Email'}
                        style={{
