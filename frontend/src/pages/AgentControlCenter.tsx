...
import { triggerEscalationScan, getAlerts } from '../services/notificationService';
...
import type { Agent, AgentActivityLog, AgentOverviewStats } from '../types/agent';
import type { PipelineExecution, QueueMetrics } from '../types/agent';
import type { AnomalyRecord } from '../types/anomaly';
import type { StandardPlaybook } from '../types/playbook';
import type { EnrichedAlert } from '../types/notification';
...
const AGENT_THEMES: Record<string, {
  ...
  metricIcon1: React.ReactNode;
  metricIcon3: React.ReactNode;
}> = {
...
const AgentSparklineWave = ({ color = '#6366f1', score = 90, id = 'agent' }: { color?: string; score?: number; id?: string }) => {
...
export const AgentControlCenter = () => {
  ...
  const [overview, setOverview] = useState<AgentOverviewStats>({});
  const [, setActivity] = useState<AgentActivityLog[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedAgentForHistory, setSelectedAgentForHistory] = useState<string | null>(null);
  ...
  const [expandedHistoricalPipelineId, setExpandedHistoricalPipelineId] = useState<string | number | null>(null);
  const [expandedHistoricalPipelines, setExpandedHistoricalPipelines] = useState<Record<string | number, PipelineExecution>>({});
  const [inspectingPipelineId, setInspectingPipelineId] = useState<string | number | null>(null);

  // Target Modal & Batch Processing State
  const [targetModalWorkflow, setTargetModalWorkflow] = useState<string | null>(null);
  ...
  // Escalation & Anomaly states
  const [authErrorToast, setAuthErrorToast] = useState<{ title: string; badge: string; message: string; hint?: string } | null>(null);
  const [anomalyFlags, setAnomalyFlags] = useState<AnomalyRecord[]>([]);
  const [actioningAnomalyId, setActioningAnomalyId] = useState<number | string | null>(null);
  const [dismissModalId, setDismissModalId] = useState<number | string | null>(null);
  const [dismissReason, setDismissReason] = useState('');
  const [expandedAnomalyId, setExpandedAnomalyId] = useState<number | string | null>(null);
  const [showPlaybooksModal, setShowPlaybooksModal] = useState(false);
  const [standardPlaybooks, setStandardPlaybooks] = useState<StandardPlaybook[]>([]);
  ...
  const fetchControlCenterData = async (isBackground = false) => {
    try {
      ...
      getStandardPlaybooks().then(pbs => {
        if (Array.isArray(pbs) && pbs.length > 0) setStandardPlaybooks(pbs as unknown as StandardPlaybook[]);
      }).catch(() => {});

      const [statusData, activityData, , , pipelinesRes, queueRes, anomalyRes] = await Promise.all([
        getAgentStatus(),
        getRecentActivity(25),
        getLatestPortfolioSnapshot().catch(() => ({ data: null })),
        getAlerts({ status: 'pending', limit: 10 }).catch(() => ({ data: { data: [] as EnrichedAlert[] } })),
        getPipelineExecutions({ page: 1, limit: 15 }).catch(() => ({ data: [] as PipelineExecution[] })),
        getQueueStatus().catch(() => null),
        getAnomalyList({ status: 'pending', limit: 20 }).catch(() => ({ data: [] as AnomalyRecord[] })),
      ]);

      if (statusData) {
        const rawStatus = statusData as { overview?: AgentOverviewStats; agents?: Agent[] };
        setOverview(rawStatus.overview || {});
        if (Array.isArray(rawStatus.agents) && rawStatus.agents.length > 0) {
          setAgents(prev => prev.map(a => {
            const found = rawStatus.agents?.find((item: Agent) => item.id === a.id);
            return found ? { ...a, ...found } : a;
          }));
        } else if (Array.isArray(statusData)) {
          setAgents(prev => prev.map(a => {
            const found = (statusData as Agent[]).find((item: Agent) => item.id === a.id);
            return found ? { ...a, ...found } : a;
          }));
        }
      }
      ...
      if (activityData) {
        setActivity(activityData as AgentActivityLog[]);
      }

      if (pipelinesRes?.data) {
        setPipelineHistory(pipelinesRes.data as PipelineExecution[]);
      }

      if (queueRes) {
        setQueueMetrics(queueRes as QueueMetrics);
      }

      if (anomalyRes?.data) {
        const rawAnomalies = Array.isArray(anomalyRes.data) ? anomalyRes.data : ((anomalyRes.data as { data?: AnomalyRecord[] })?.data || []);
        setAnomalyFlags(rawAnomalies as AnomalyRecord[]);
      }
    } catch (err) {
  ...
  useEffect(() => {
    void fetchControlCenterData(false);

    const socketInstance = connectSocket();
    const handleEvent = () => {
      void fetchControlCenterData(true);
    };
    ...
    if (socketInstance) {
      ...
      socketInstance.on('ANOMALY_DETECTED', (payload: {
        ...
        setAnomalyFlags(prev => {
          if (prev.some(f => f.payment_id === payload.payment_id)) return prev;
          const newFlag: AnomalyRecord = {
          ...
          return [newFlag, ...prev].slice(0, 20);
        });
      });
    }
  ...
  const handleDismissAnomaly = async (anomalyId: number | string) => {
  ...
  const handleEscalateAnomaly = async (anomalyId: number | string) => {
    setActioningAnomalyId(anomalyId);
    try {
  ...
  const handleOpenTargetModal = async (workflowName: string) => {
    if (isViewer) {
      setAuthErrorToast({
    ...
    try {
      ...
      const targetList = (Array.isArray(targets) ? targets : []) as PendingTarget[];
      setPendingTargets(targetList);
  ...
  const handleExecuteSingleTarget = async () => {
    ...
    try {
      ...
      const msg = (err as { response?: { data?: { message?: string } } })?.response?.data?.message || 'Failed to trigger multi-agent pipeline workflow.';
      setAuthErrorToast({
  ...
  const handleInspectHistoricalPipeline = async (pipelineId: string | number, forceRefresh = false) => {
  ...
  const handleTriggerAgent = async (agentIdStr: string) => {
    ...
    try {
      ...
      if (status === 403 || status === 401) {
        setAuthErrorToast({
          ...
          message: (err as { response?: { data?: { message?: string } } })?.response?.data?.message || `You do not have required operational permissions to run ${agentName}.`,
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        <div className="pipelines-grid">
          ...
          <div style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '8px' }}>
              <button
                onClick={() => void handleOpenTargetModal('RECONCILIATION_AND_RISK')}
          ...
          <div style={{
            ...
            <button
              onClick={() => void handleTriggerPortfolioDirect()}
          ...
          <div style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '8px' }}>
              <button
                onClick={() => void handleOpenTargetModal('END_TO_END_COMPLIANCE')}
                disabled={triggeringPipeline || batchRunning}
                style={{
              ...
              <button
                onClick={() => void handleExecuteBatch('END_TO_END_COMPLIANCE')}
                disabled={batchRunning}
                style={{
                  background: '#f8fafc',
                  color: '#7c3aed',
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              <button
                onClick={() => void handleExecuteBatch(targetModalWorkflow)}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                <button
                  onClick={() => void handleExecuteSingleTarget()}
                  disabled={triggeringPipeline || batchRunning}
                  style={{
                    display: 'flex', alignItems: 'center', gap: '6px',
                    background: 'linear-gradient(135deg, #4f46e5 0%, #3730a3 100%)',
      ...
      {/* AGENT PERFORMANCE TELEMETRY SECTION */}
          ...
          {agents.map((agentItem) => {
            ...
            return (
              ...
              >
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '8px' }}>
                  <button
                    onClick={() => void handleTriggerAgent(agentItem.id)}
                ...
                <div>
                  <div style={{ fontSize: '1.1rem', fontWeight: '900', color: '#0f172a', lineHeight: 1.1 }}>
                    {(() => {
                      ...
                      const queueActive = queueMetrics?.activeJobsCount ?? queueMetrics?.active ?? 0;
                    ...
                    <React.Fragment key={p.id}>
                      ...
                      >
                        ...
                        <td style={{ padding: '14px 20px', textAlign: 'right' }}>
                          <button
                            onClick={() => void handleInspectHistoricalPipeline(p.id)}
                            style={{
                          ...
                          <td colSpan={6} style={{ padding: '12px 20px 24px 20px' }}>
                            {isLoadingThis && !detail ? (
                              ...
                              <PipelineVisualizer
                                ...
                                onRefresh={() => void handleInspectHistoricalPipeline(p.id, true)}
                              />
                            )}
                          </td>
                        </tr>
      ...
      <div style={{ background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 1px 6px rgba(0,0,0,0.06)', marginBottom: '32px' }}>
        <div style={{ background: 'linear-gradient(135deg, #f59e0b 0%, #d97706 100%)', padding: '20px 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            ...
            <button onClick={() => void fetchControlCenterData()} style={{ background: 'rgba(255,255,255,0.2)', border: 'none', borderRadius: '8px', padding: '7px 10px', color: '#ffffff', cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
              <RefreshCw size={14} />
            </button>
          </div>
        </div>
        ...
        {anomalyFlags.length === 0 ? (
          ...
          <div style={{
            ...
            {anomalyFlags.map(flag => {
              const severityConfig: Record<string, { bg: string; border: string; badge: string; label: string; icon: React.ReactNode }> = {
              ...
              const actionConfig: Record<string, { bg: string; text: string; border: string; label: string }> = {
              ...
              const cfg = severityConfig[flag.severity || 'LOW'] || severityConfig.LOW;
              const actCfg = actionConfig[flag.recommended_action || 'REVIEW'] || actionConfig.REVIEW;
              ...
              const checklist = Array.isArray(ev.action_checklist) && (ev.action_checklist as string[]).length > 0
                ? (ev.action_checklist as string[])
                      ...
                      <div style={{ minWidth: 0 }}>
                        ...
                        <div style={{ margin: '3px 0 0', fontSize: '0.73rem', color: '#64748b', display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
                          ...
                          <span style={{ fontWeight: '700', color: '#334155' }}>
                            · Score: {parseFloat(String(flag.anomaly_score || 0)).toFixed(0)}/100
                          </span>
                        </div>
                      </div>
                        ...
                        {!isEscalated && (
                          <>
                            ...
                            {(flag.severity === 'HIGH' || flag.severity === 'CRITICAL' || flag.recommended_action === 'ESCALATE') && (
                              <button
                                onClick={() => void handleEscalateAnomaly(flag.id)}
                                disabled={Boolean(isActioning)}
                      ...
                      <div style={{
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: '0.95rem', fontWeight: '800', color: '#0f172a', marginTop: '2px' }}>
                            ₹{Number(ev.payment_amount || 0).toLocaleString('en-IN')}
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: '0.95rem', fontWeight: '800', color: '#0f172a', marginTop: '2px' }}>
                            ...
                            {Boolean(ev.payment_vs_emi_ratio) && (
                              <span style={{ fontSize: '0.72rem', color: Number(ev.payment_vs_emi_ratio) > 3 ? '#dc2626' : '#2563eb', marginLeft: '6px', fontWeight: '700' }}>
                                ({String(ev.payment_vs_emi_ratio)}× EMI)
                              </span>
                            )}
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: '0.95rem', fontWeight: '800', color: '#0f172a', marginTop: '2px' }}>
                            ...
                            {Boolean(ev.payment_vs_outstanding_ratio) && (
                              <span style={{ fontSize: '0.72rem', color: Number(ev.payment_vs_outstanding_ratio) > 1.2 ? '#ea580c' : '#059669', marginLeft: '6px', fontWeight: '700' }}>
                                ({String(ev.payment_vs_outstanding_ratio)}× Bal)
                              </span>
                            )}
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: '0.78rem', fontWeight: '700', color: ev.payer_account === ev.registered_account ? '#166534' : '#991b1b', marginTop: '2px' }}>
                            {String(ev.payer_account || 'N/A')} {ev.registered_account ? `(Reg: ${String(ev.registered_account)})` : ''}
                          </div>
                        </div>
                      </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', backdropFilter: 'blur(4px)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#ffffff', borderRadius: '20px', padding: '32px', width: '440px', boxShadow: '0 25px 80px rgba(0,0,0,0.25)' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', marginTop: '16px', justifyContent: 'flex-end' }}>
              ...
              <button
                onClick={() => void handleDismissAnomaly(dismissModalId)}
                  ...
                  <div style={{ background: '#f8fafc', border: '1px solid #f1f5f9', borderRadius: '10px', padding: '10px 14px' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                      {pb.steps && pb.steps.map((s, idx) => (
                        <div key={s.id || idx} style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', fontSize: '0.76rem' }}>
                          <span style={{ fontWeight: '800', color: '#4f46e5' }}>{s.id || idx + 1}.</span>
                          <div>
                            <strong style={{ color: '#1e293b' }}>{s.label || s.title || `Step ${idx + 1}`}: </strong>
                            <span style={{ color: '#64748b' }}>{s.desc || s.description || ''}</span>
                          </div>
                        </div>
                      ))}
                    </div>
