...
import { X, Activity, CheckCircle2, AlertCircle, Layers, Terminal, RefreshCw, ChevronDown, ChevronUp } from 'lucide-react';
...
interface StepLogOutput {
  breached_borrowers?: Array<{ company?: string; company_name?: string; company_id?: number; overdue_amount?: string | number; outstanding_amount?: string | number; overdue_days?: number | string }>;
  total_delinquent_companies?: number;
  companies?: Array<{ company_id?: number; overdue_days?: number | string }>;
  breach_count?: number;
  total_alerts?: number;
  critical?: number;
  high?: number;
...
interface StepLog {
  ...
  status: string;
  duration_ms?: number;
  output_data?: string | StepLogOutput | null;
}

interface AgentRunHistoryDrawerProps {
  agent?: AgentObject | string;
  agentId?: string;
  agentName?: string;
  onClose: () => void;
}
 ...
 */
export const AgentRunHistoryDrawer = ({ agent, agentId, agentName, onClose }: AgentRunHistoryDrawerProps) => {
  const currentAgentId = (typeof agent === 'object' ? agent?.id : agent) || agentId || '';
  const currentAgentName = (typeof agent === 'object' ? agent?.name : undefined) || agentName || (typeof agent === 'string' ? agent : 'Operational Agent');
  const agentMetrics = typeof agent === 'object' ? agent?.metrics : undefined;

  const [runs, setRuns] = useState<AgentRun[]>([]);
  const [loading, setLoading] = useState(true);
  const [expandedRunId, setExpandedRunId] = useState<string | number | null>(null);
  const [runLogs, setRunLogs] = useState<Record<string | number, StepLog[]>>({});
  const [loadingLogs, setLoadingLogs] = useState<Record<string | number, boolean>>({});
  ...
  useEffect(() => {
    if (currentAgentId) {
      ...
      if (socket) {
        const handleRefresh = () => {
          getAgentRuns(currentAgentId, 30)
            .then(data => {
  ...
  const toggleExpandRun = async (runId: string | number) => {
    ...
    if (!runLogs[runId]) {
      try {
        ...
        const rawLogs = await getRunDetail(currentAgentId, runId);
        const logs = (Array.isArray(rawLogs) ? rawLogs : []) as StepLog[];
          ...
          {agentMetrics && (
            <div style={{
              ...
              <div>
                ...
                <div style={{ fontSize: '1.2rem', fontWeight: '800', color: '#0f172a' }}>{agentMetrics.total_runs || 0}</div>
              </div>
              <div>
                <div style={{ fontSize: '0.7rem', fontWeight: '600', color: '#64748b' }}>AVG CONFIDENCE</div>
                <div style={{ fontSize: '1.2rem', fontWeight: '800', color: '#059669' }}>{agentMetrics.avg_confidence ? `${parseFloat(String(agentMetrics.avg_confidence)).toFixed(1)}%` : '0%'}</div>
              </div>
              <div>
                <div style={{ fontSize: '0.7rem', fontWeight: '600', color: '#64748b' }}>TOTAL TOKENS</div>
                <div style={{ fontSize: '1.2rem', fontWeight: '800', color: '#4f46e5' }}>{parseInt(String(agentMetrics.total_tokens || 0), 10).toLocaleString()}</div>
              </div>
            </div>
          )}
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: '0.7rem', color: '#64748b', marginTop: '2px' }}>
                            {formatAuditTimestamp(run.created_at)} • Triggered by: {(run as unknown as { triggered_by_name?: string }).triggered_by_name || 'System'}
                          </div>
                        </div>
                      </div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        <div style={{ textAlign: 'right' }}>
                          <div style={{ fontSize: '0.8rem', fontWeight: '700', color: (run.confidence_score ?? 0) >= 80 ? '#059669' : '#b45309' }}>
                      ...
                      <div style={{ borderTop: '1px solid #e2e8f0', padding: '16px', background: '#f8fafc', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '6px' }}>
                          ...
                          {loadingLogs[run.id] ? (
                            ...
                            logs.map((step, idx) => {
                              let out: StepLogOutput | null = null;
                              if (typeof step.output_data === 'string') {
                                try {
                                  out = JSON.parse(step.output_data) as StepLogOutput;
                                } catch {
                                  out = null;
                                }
                              } else if (typeof step.output_data === 'object' && step.output_data !== null) {
                                out = step.output_data as StepLogOutput;
                              }
                                    ...
                                    <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '8px 10px', marginTop: '2px' }}>
                                      {out.breached_borrowers && Array.isArray(out.breached_borrowers) ? (
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '0.72rem' }}>
                                          ...
                                          {(out.critical ?? 0) > 0 && <span style={{ color: '#dc2626', fontWeight: '700' }}>Critical: {out.critical}</span>}
                                          {(out.high ?? 0) > 0 && <span style={{ color: '#d97706', fontWeight: '700' }}>High: {out.high}</span>}
                                        </div>
