...
import { CreditCard, Zap, CheckCircle2, Shield, Mail, X, Sparkles, Activity } from 'lucide-react';
 ...
 * Automatically notifies dashboard views to refresh table data in real-time across connected browsers.
 */
export const LiveToastNotifications = ({ onRealtimeUpdate }) => {
  const [toasts, setToasts] = useState([]);

  useEffect(() => {
    const socket = connectSocket();

    const addToast = (type, title, message, details = null) => {
      const id = Date.now() + Math.random();
      const newToast = { id, type, title, message, details, time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' }) };
    ...
    const handlePaymentIngested = (data) => {
      const p = data.payment || {};
      addToast(
        'payment',
        '💳 New Bank Deposit Ingested',
        `Received ₹${parseFloat(p.amount || 0).toLocaleString('en-IN')} from ${p.sender_name || 'Sender'}. Case #${data.case?.id} opened in state NEW.`,
        data
      );
    };

    // 2. Reconciliation Started Event
    const handleReconcileStarted = (data) => {
      addToast(
        'started',
    ...
    const handleReconcileCompleted = (data) => {
      const score = data.recommendation ? parseFloat(data.recommendation.confidence_score).toFixed(0) : '90';
      addToast(
        'completed',
    ...
    const handleRiskCompleted = (data) => {
      addToast(
        'risk',
    ...
    const handleCollectionDrafted = (data) => {
      addToast(
        'collection',
