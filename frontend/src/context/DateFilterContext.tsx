...
export type { DatePreset, DateRange } from '../types/common';
...
export const calculateDateBounds = (preset: DatePreset): DateRange => {
  const now = new Date();
...
export const formatDisplayLabel = (preset: DatePreset, startDate: string, endDate: string): string => {
  ...
  switch (preset) {
    ...
    case '7d': return `Last 7 Days (${formatShort(startDate)} – ${formatShort(endDate)})`;
    case 'this_month': return 'This Month';
    case '30d': return 'Last 30 Days';
    case '2026': return 'FY 2026';
    case '2025': return 'FY 2025';
    case 'ytd':
    case 'this_year': return 'Year-to-Date (YTD)';
    case 'custom': return `${formatShort(startDate)} – ${formatShort(endDate)}`;
    default: return startDate && endDate ? `${formatShort(startDate)} – ${formatShort(endDate)}` : 'All Time';
  }
};
...
export const DateFilterProvider = ({ children }: { children: ReactNode }) => {
  const [activePreset, setActivePreset] = useState<DatePreset>(() => {
    try {
      ...
      return (saved as DatePreset) || 'all';
    } catch {
      return 'all';
    }
  });
  ...
  const setCustomRange = (start: string, end: string): void => {
